...
class ActorCritic:
    def __init__(self, env, sess):
        ...
        # TODO - there are shape issue that we will need to figure out
    ...
    def create_actor_model(self):
        ...
        h1 = LSTM(64,return_sequences=True, stateful=False, dropout=0.2, activation='relu')(state_input)
        h2 = LSTM(64, dropout=0.2, return_sequences=True, activation='relu')(h1)
        h3 = LSTM(64, dropout=0.2, return_sequences=True, activation='relu')(h2)
        output = Dense(2, kernel_initializer='lecun_uniform', activation='relu')(h3)
    ...
    def create_critic_model(self):
        state_input = Input(shape=self.env.observation_space.shape[0:])
        state_h1 = LSTM(64,return_sequences=True, stateful=False, dropout=0.2, activation='relu')(state_input)
        state_h2 = LSTM(64, dropout=0.2, return_sequences=True, activation='relu')(state_h1)

        action_input = Input(shape=self.env.action_space.shape) 
        action_h1 = LSTM(64, dropout=0.2, return_sequences=True, activation='relu')(action_input)
...
filename = 'UDR_base_line_A2C_LSTM_V2_render.txt'

# export_summary_stat_path = './run_summary/base_line_A2C_V1_run_summary.csv'
export_summary_stat_path = './run_summary/UDR_base_line_A2C_LSTM_V2_run_summary.csv'
...
for trial in range(trials): 
    ...
    for step in range(trial_len):
        action = actor_critic.act(cur_state[0])
...
model_code = 'UDR_baseline_A2C_LSTM_V2_{0}_iterations_{1}_steps_each'.format(trials,trial_len)
